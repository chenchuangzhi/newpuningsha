/*jshint esversion: 6 */
game.import("extension", function(lib,game,ui,get,ai,_status){window.url = {url: lib.assetURL + "extension/十周年UI",};return { name:"十周年UI",
content:function(config, pack){
	'use strict';	
        delete lib.extensionMenu['extension_十周年UI'].delete;
/*-----------------分割线-----------------*/
...
*/
	...
	lib.skill._longLevel = {
		...
		content: function () {
			...
			if (lib.config.extension_十周年UI_longLevel == 'sex') {
				var rarity = ['silver', 'gold', 'yu'].randomGet();//, 'bing', 'yan'
			};
    ...
    //感谢EngJ.K大佬	
    lib.skill._check_dynamicSkin_Player={
		trigger:{
		    global:'gameStart',
		},
		forced:true,
		charlotte:true,
		filter:function(event,player){
		    return !player.doubleAvatar&&player.dynamic&&lib.config['extension_十周年UI_newDecadeStyle']=="off"
		},
		content:function(){
		    if(Object.keys(player.getElementsByClassName("skinYh")).length<=0){
		        var box=document.createElement("img");
		        box.src=url.url+"/image/skinbox/"+player.group+".png";
		        box.onerror=function(){
		            this.src=url.url+"/image/skinbox/weizhi.png";
		            this.onerror=null;
		        }
		        box.classList.add("skinYh");
		        box.style.display="block";
		        box.style.position="absolute";
		        box.style.top="-22px";
		        box.style.height="50px";
		        box.style.width="131.1px";
		        box.style.zIndex="61";
		        player.appendChild(box);
		    }
		}
    }
    lib.skill._remove_dynamicSkin_Player={
		trigger:{
		    player:'dieBegin',
		},
		forced:true,
		...
		content:function(){
		    if(Object.keys(player.getElementsByClassName("skinYh")).length>0){
		        var box=document.createElement("img");
		        box.classList.remove("skinYh");
		        var box2=player.getElementsByClassName("skinYh")[0];
		        player.removeChild(box2);
		    }
		}
    }
    lib.skill._dieAfter_dynamicSkin_Player={
		trigger:{
		    player:'dieBegin',
		},
		forced:true,
		...
		priority:-1,
		filter:function(event,player){
		    return !player.doubleAvatar&&(lib.config.extension_十周年UI_dynamicSkin_dieAfter == true)&&player.dynamic&&lib.config['extension_十周年UI_newDecadeStyle']=="off"
		},
		content:function(){
		    if(Object.keys(player.getElementsByClassName("skinYh")).length<=0){
		        var box=document.createElement("img");
		        box.src=url.url+"/image/skinbox/weizhi.png";
		        box.onerror=function(){
		            this.src=url.url+"/image/skinbox/weizhi.png";
		            this.onerror=null;
		        }
    ...
    lib.skill._revive_dynamicSkin_Player={
		trigger:{
		    player:'reviveTriggerx',//自定义时机
		},
		forced:true,
   		forceDie:true,
		charlotte:true,
		filter:function(event,player){
		    return (lib.config.extension_十周年UI_dynamicSkin_dieAfter == true)&&player.dynamic&&lib.config['extension_十周年UI_newDecadeStyle']=="off"
		},
		content:function(){
		    if(Object.keys(player.getElementsByClassName("skinYh")).length>0){
		        var box2=player.getElementsByClassName("skinYh")[0];
...
lib.element.player.revive = function (hp, log) {//修改源码，自定义时机
    ...
    // 增加内容
    let evt = game.createEvent("reviveTriggerx");
    evt.player = this;
...
//engex开始
			if(lib.config.mode!='connect' && game.getExtensionConfig('EngEX', 'enable')) {}
			else{
                            ...
                            if (!this.storage.nohp) {
                                if (hpMax > 5) {
                                    ...
                                    if (!hpNode.textstyle) {
                                        ...
                                        hpNode.appendChild(hpNode.$hpText)
                                        hpNode.appendChild(hpNode.$br1)
                                        hpNode.appendChild(hpNode.$slash)
                                        hpNode.appendChild(hpNode.$br2)
                                        hpNode.$hpMaxText.textContent = hpMaxText;
                                        hpNode.appendChild(hpNode.$hpMaxText)
                                        hpNode.appendChild(hpNode.$div)
                                    }
							
                                    if (hp == 0)
                                        hpNode.lastChild.classList.add('lost');
                                    else
                                        hpNode.lastChild.classList.remove('lost');
...
//结束					
				...
				//-----------------分割线-----------------
				// 新样式装备栏，Engj.K大佬提供代码
			if (!(lib.config['extension_EngEX_SSEquip'])) {
				if (lib.config.extension_十周年UI_equipStyle == 'off') {
					...
					if (!(ele.length > 1)) {
						...
						if (subype == "equip3" || subype == "equip4") {
                            var b = subype == "equip3" ? "+" : "-";
                            ele[0].style.backgroundImage = "url(" + decadeUIPath + "/image/" + b + "1.png)";
                            ele[0].style.marginLeft = "12px";
                            //ele[0].classList.add("box");
                        }
						...
						for (i = 0; i < e.length; i++) {
							...
							e[i].style.marginLeft = "13px"; // 均右移
							} else {
								e[i].style.color = "#E9E8E3"; // 装备字体颜色显示
								e[i].style.marginLeft = "3px"; // 装备字体单独右移
							}
                                            ...
                                            if (!(e[0].nodeName == "IMG")) {
                                                ...
                                                if (subype) {
                                                    if (eng.SSEquip) {
                                                        ...
                                                        if (eng.SSEquip[t]) {
                                                            e[2].textContent = eng.SSEquip[t]
                                                            if (!(subype == "equip3" || subype == "equip4")) {
                                                                e[0].setAttribute("src", eng.url + "/images/ass/" + cardName + ".png")
						...
						mark:function(item, info, skill){
							...
							if (get.itemtype(item) == 'card') {
								...
								if (!markText || markText[0] == '+' || markText[0] == '-') {
									...
									if (decadeUI.config.playerMarkStyle != 'decade'&&decadeUI.config.playerMarkStyle != 'shizhounian') {
										markText = markText[0];
									}
								}
...
//结束						
						...
						*/
						...
						$dieAfter:function(){
							if ( lib.config.extension_十周年UI_dynamicSkin_dieAfter != true ) this.stopDynamic();
					...
					// num = 0 全部不显示
						...
						if (lib.config.extension_十周年UI_newDecadeStyle == 'othersOn') {
							...
							setTimeout(function(){
								...
								if(decadeUI.config.playerDieEndEffect){
...
//无名补丁开始
								//intro.innerText = '切换';
								intro.innerHTML = '<button type="submit" id="but1"></button>';
								intro._node = node;
...
//结束
				...
				if (!lib.card[viewAs] || !lib.card[viewAs].effect){
					...
					if(cards[0].viewAs=='bingliang' || cards[0].viewAs=='lebu' || cards[0].viewAs=='shandian'){
						...
						if(judgeText=="兵" || judgeText=="乐" || judgeText=="闪"){
						    var newj=cards[0].node.judgeMark.node.judge;
							newj.innerHTML="";
							// var name = newj.parentElement.parentElement.getAttribute("data-card-name");
							newj.classList.add("newjudge");
							newj.style.backgroundImage='url("'+lib.assetURL+"extension/十周年UI/assets/image/"+ map[judgeText] +".png"+'")';
						 /*   newj.style.height = '80px';
							newj.style.width = '80px';
							//newj.style.left = '-50px';
							newj.style.marginLeft = '-50px ';*/
							newj.style.marginTop = '5px ';
							newj.style.zIndex = '99';
							newj.parentElement.children[0].style.background="none";
							newj.parentElement.children[0].style.display="none";
						}
					}
...
//engex开始(未安装engex不生效)
				        if (lib.config['extension_EngEX_SSEquip']) {
                            ...
                            if (!(ele.length > 1)) {
                                ...
                                if (lib.config['extension_EngEX_SSEquipEffect'] == "text") {
                                    if (b) e[1].textContent = "已废除";
                                    ...
                                    if (b) img.src = eng.url + "/images/ass/feichuma.png";
                                    else img.src = eng.url + "/images/ass/feichu.png";
...
config:{
	...
	dynamicBackground:{
		...
		item:{
			...
			skin_chengzhu_城主边框:				'城主金龙边框(需自行调参数)',
	...
	dynamicSkin:{
		name: '动态皮肤(会盖住千幻)',
        "intro": "部分老手机上无法显示，默认关闭",
		init: self.OffscreenCanvas == undefined ? false : true,
        onclick: function () {
            if ( lib.config.extension_十周年UI_dynamicSkin == false ) {
                ...
                if(self.OffscreenCanvas == undefined) {
                    alert("您的设备较老，动皮可能无法正常显示。");
                }
            }
            else game.saveConfig('extension_十周年UI_dynamicSkin', false);            
        }
	},
...
/*	showJieMark:{
	...
	newDecadeStyle: {
		name: '<b><font color=\"#FF0000\">武将边框样式(需重启)',
	...
	shadowStyle: {
		name: '<b><font color=\"#FF9000\">切换特效样式',
	...
	decadeLayout: {
		name: '<b><font color=\"#0040FF\">武将位置布局',
	...
	loadingStyle: {
		name: '<b><font color=\"#FF6020\">更换窗口边框效果',
...
package:{
	...
	author:"短歌 <br> 修改：南尋<br> 更新：Show-K",
	diskURL:"",
	forumURL:"",
	version:"1.2.0.220114",
	intro: "2022.11.14日，子琪使用Show-K修复版十周年UI的代码进行更新，修复护甲等BUG。<br> 此扩展包含动态皮肤210个，其中150个为子琪选自『梦终』资源包（已征得作者之一'梦寻千古鹿原雪'同意），选出的动皮与『特效测试』扩展的出框特效一致。<br> 『梦终』资源包禁止外传！（作者:缘空、梦寻千古鹿原雪）请加绘声群看运气获取。『绘声』无名杀交流群Ⅰ:343332101，群Ⅱ:333180740，群Ⅲ:743463033。",
},
files:{
	"character":[],
	"card":[],
